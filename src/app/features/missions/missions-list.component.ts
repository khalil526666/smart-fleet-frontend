...
@Component({
  ...
  styles: [`
    ...
    .chip { border: 1px solid var(--border); background: var(--bg-surface-2); color: var(--text-primary); padding:.35rem .7rem; border-radius:999px; cursor:pointer; font-weight:700; font-size:.85rem; }
    .chip.on { background: var(--accent); color: var(--bg-app); border-color: var(--accent); }
    ...
    .modal { position: fixed; top: 12vh; left: 50%; transform: translateX(-50%); width: min(720px, calc(100vw - 24px)); background: var(--bg-surface); border-radius: 16px; border: 1px solid var(--border); box-shadow: 0 30px 80px rgba(0,0,0,.6); padding: 14px 16px; z-index: 30; color: var(--text-primary); }
    .modal-head { display:flex; justify-content:space-between; align-items:center; margin-bottom: 10px; }
    .modal-title { font-weight: 900; font-size: 1.05rem; color: var(--text-primary); }
    .icon-x { width: 34px; height: 34px; border-radius: 12px; border: 1px solid var(--border); background: var(--bg-surface-2); color: var(--text-primary); cursor:pointer; font-size: 18px; }
    .grid { display:grid; grid-template-columns: 1fr 1fr; gap: .75rem; }
    .field { display:flex; flex-direction:column; gap:.35rem; font-weight: 750; font-size: .9rem; color: var(--text-primary); }
    input, textarea, select { border: 1px solid var(--border); border-radius: 10px; padding: .55rem .6rem; font: inherit; background: var(--bg-surface-2); color: var(--text-primary); }
    textarea { resize: vertical; }
    .modal-actions { margin-top: 12px; display:flex; justify-content:flex-end; gap:.5rem; }
    .modal-hint { margin: 0 0 12px; padding: 10px 12px; background: rgba(63,185,80,.08); border-radius: 12px; color: var(--accent); font-size: .9rem; line-height: 1.45; }
